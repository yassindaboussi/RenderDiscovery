    ...
    bcrypt.compare(password,user.password,async function(err,result){
      ...
      if(result){
        bcrypt.hash(newPassword, 10, (err, hash) => {
          ...
          res.status(401).json('Password does not matched')
      }
  })
...
const VerifCodeForgot = async (req, res, next) => {
  ...
  if (!email || !codeForget) {
    ...
    const user = await User.findOne({ email: req.body.email.toLowerCase() });
    console.log(req.body.email);
    console.log("Code enter by the User ==> " + user);
    ///
    if (req.body.codeForget == user.codeForget && user.codeForget != "empty") {
  ...
  if (!userMail) {
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      if (error) {
        ...
        userMail.codeForget=RandomXCode;
        userMail.save()
      }
...
const EditProfil = async (req, res, next) => {
  const { username, email, password, bio,newPassword } = req.body;
